...
const Testimonials = () => {
  ...
  useEffect(() => {
    fetch("https://bistro-boss-server-tau-six.vercel.app/api/v1/reviews")
      .then((res) => res.json())
      .then((data) => {
        setRevies(data);
      });
      ...
      <Swiper navigation={true} modules={[Navigation]} className="mySwiper">
        {reviews?.map((review) => (
          <SwiperSlide key={review._id}>
            <div className="max-w-5xl mx-auto flex flex-col items-center text-center gap-10 mt-10">
              <Rating
                style={{ maxWidth: 250 }}
                value={review.rating}
                readOnly
              ...
              <h4 className="text-3xl font-medium text-yellow-500">
                {review.name}
              </h4>
            </div>
          </SwiperSlide>
